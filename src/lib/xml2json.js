...
export default function xml2json(xml, tab) {
  var X = {
    toObj: function (xml) {
          ...
          if (hasElementChild) {
            if (textChild < 2 && cdataChild < 2) {
              ...
              for (var n1 = xml.firstChild; n1; n1 = n1.nextSibling) {
                if (n1.nodeType == 3)
                  // text node
                  o['#text'] = X.escape(n1.nodeValue);
                else if (n1.nodeType == 4)
                  // cdata node
                  o['#cdata'] = X.escape(n1.nodeValue);
                else if (o[n.nodeName]) {
                  // multiple occurence of element ..
                  if (o[n1.nodeName] instanceof Array)
                    o[n1.nodeName][o[n1.nodeName].length] = X.toObj(n1);
                  else o[n1.nodeName] = [o[n1.nodeName], X.toObj(n1)];
                } // first occurence of element..
                else o[n1.nodeName] = X.toObj(n1);
              }
            } else {
              // mixed content
      ...
      } else throw('unhandled node type: ' + xml.nodeType);
      return o;
    },
